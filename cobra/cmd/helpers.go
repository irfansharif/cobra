...
func guessProjectPath() {
	...
	if strings.ContainsRune(inputPath, os.PathSeparator) {
		...
		case 1:
			projectPath = findProjectPath("github.com")
			return
		case 2:
			for _, srcPath := range srcPaths {
		...
		if projectBase == "" {
			...
			projectPath = findProjectPath(projectBase)
			return
		}
	}
}

func findProjectPath(projectBase string) string {
	srcPaths := getSrcPaths()
	for _, srcPath := range srcPaths {
	...
	return filepath.Join(srcPaths[0], projectBase, inputPath)
}
